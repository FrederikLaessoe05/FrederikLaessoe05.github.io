...
function update() {
    ...
    // spawn new coins
    if(gameState == "action" &&
    coinTimeSinceLastSpawn>coinSpawnInterval*2) {
        coins.push({
            xPosition: spawnXPosition,
    ...
    //draw the highscore
    drawImage(highscoreimage,
        highscoreXposition,
        highscoreYposition,
        highscoreimage.width,
        highscoreimage.height

    );
    drawText(
        highscorevalue,
        highscoreTextXPosition,
        highscoreTextYPosition,
        highscoretextsize,
        highscoretextcolor
    );

if(gameState == "action") {
    highscorevalue+=timeSinceLastFrame/1000;
}
    // draw the menu text
    if(gameState == "menu") {
        drawText (
